...
| Method | Path | Notes |
|---|---|---|
| POST | `/api/import/mixs` | Multipart with `file` (xlsx or tsv), `projectId`, `dryRun`, `columnMap`, `siteMatchKm`, `allowSitesWithoutCoords`, `people`, `defaultChecklist`, `defaultExtension`. See [MIxS pipeline](mixs.md). Rate-limited 1/sec/IP |
