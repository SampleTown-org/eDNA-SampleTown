...
import { deleteProjectSubtree } from '$lib/server/project-delete';
...
/**
 * Hard-delete a project and everything belonging to it. The ordering and the
 * reasoning behind it live in `project-delete.ts`, shared with the preview so
...
export const DELETE: RequestHandler = async ({ params, locals }) => {
	try {
		...
		return json({ ok: true, deleted: deleteProjectSubtree(db, projectId) });
	} catch (err) {
		return apiError(err);
	}
};
