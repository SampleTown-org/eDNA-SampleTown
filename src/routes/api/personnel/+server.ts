...
export const GET: RequestHandler = async ({ url, locals }) => {
	...
	const query = activeOnly
		? 'SELECT p.*, u.username AS github_username, u.avatar_url, u.avatar_emoji FROM personnel p LEFT JOIN users u ON u.id = p.user_id WHERE p.is_active = 1 AND p.lab_id = ? ORDER BY p.sort_order, p.full_name'
	...
	return json(db.prepare(query).all(labId));
};
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	try {
		db.prepare(
			`INSERT INTO personnel (id, lab_id, full_name, email, role, institution, orcid, user_id, sort_order)
			 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
		).run(
			id,
			labId,
