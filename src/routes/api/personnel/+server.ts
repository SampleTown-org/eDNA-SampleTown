...
export const POST: RequestHandler = async ({ request }) => {
	...
	const id = generateId();
	try {
		db.prepare(`INSERT INTO personnel (id, full_name, email, role, institution, orcid, user_id, sort_order)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(
			id, data.full_name, data.email || null, data.role || null,
			data.institution || null, data.orcid || null, data.user_id || null,
			data.sort_order ?? 0
		);
		return json(db.prepare('SELECT p.*, u.username AS github_username FROM personnel p LEFT JOIN users u ON u.id = p.user_id WHERE p.id = ?').get(id), { status: 201 });
	} catch (err) {
		return apiError(err);
	}
};
