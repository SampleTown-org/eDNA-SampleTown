...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	// Coerce empty strings to null for CHECK constraints
	const platform = data.platform || null;
	const libraryType = data.library_type || '16S_amplicon';

	try {
		db.prepare(`
			INSERT INTO library_plates (id, plate_name, library_prep_date, library_type, library_prep_kit,
				platform, instrument_model, fragment_size_bp, pcr_plate_id, notes, custom_fields, created_by)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		`).run(plateId, data.plate_name, data.library_prep_date || null, libraryType,
			data.library_prep_kit || null, platform, data.instrument_model || null,
